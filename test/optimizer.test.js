...
const myHeader = new ast.Function("myName", "x", "int");
const myFunction = new ast.FunctionDeclaration(myHeader, []);
const callMyFunction = (args) => new ast.FuncCall(myFunction, args);
const intFun = body => new ast.FunctionDeclaration("f", [], "int", body)
const or = (...d) => d.reduce((x, y) => new ast.BinaryExpression("||", x, y));
const and = (...c) => c.reduce((x, y) => new ast.BinaryExpression("andIoop", x, y));
...
const tests = [
  ...
  ["optimzies negation", new ast.NegExpression("youreNotMyDad", true), false],
  ["passes through no negation", new ast.NegExpression("", true), true],
  ...
  [
    ...
    new ast.FunctionDeclaration(new ast.Function("myFunc", new ast.Parameter(5 + 7, "x"), "int"), []),
    new ast.FunctionDeclaration(new ast.Function("myFunc", new ast.Parameter(12, "x"), "int"), [])
  ],
  ["optimizes array elements", new ast.ArrayExpression([7+5, 12, 1]), new ast.ArrayExpression([12, 12, 1])],
  [
